...
def hist_xs(df, tag, nbins, ax):
    ax.hist(df[tag][df.label==0], bins=nbins, histtype=u'step', label='background')
    ax.hist(df[tag][df.label==1], bins=nbins, histtype=u'step', label='signal')
    plt.xlabel(tag)
    ax.legend(loc='upper right')
    return

def hist_ms(df, min, max, nbins, ax, sig_limits=None):
    ax.hist(df['m'][df.label==0], range=(min, max), bins=nbins, histtype=u'step', label='background')
    ax.hist(df['m'][df.label==1], range=(min, max), bins=nbins, histtype=u'step', label='signal')
...
def hist_cut_ms(df, opt_df, min, max, nbins, ax, sig_limits=None):
    # signal
...
def hist_softmax_cut_ms(df, min, max, nbins, ax, sig_limits=None):
    # signal
