...
class ModClf:
    ...
    def set_test_fraction(self, test_frac):
    ...
    def set_test_masses(self, masses):
        self.test_masses = masses

    def gen_train_peak_mask(self):
        self.train_peak_mask = self.make_peak_mask(self.train_masses)

    def gen_test_peak_mask(self):
        self.test_peak_mask = self.make_peak_mask(self.test_masses)
    ...
    def make_peak_mask(self, masses):
        ...
        peak_idxs = np.where(np.abs(masses-self.mass_mean) <= self.mass_width)
...
class ModBinarySoftmaxClf(ModClf):
    ...
    def set_signif_type(self, type_str):
        if type_str not in ['proba']: # , 'categ']:
    ...
    def loss_object(self, y_truth, y_pred, train=True):
        ...
        return tf.keras.losses.categorical_crossentropy(y_truth, y_pred) + tf.math.divide(1, signif)
