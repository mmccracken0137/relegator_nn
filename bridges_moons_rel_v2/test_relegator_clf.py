...
fom_mean = train_df[fom_name][train_df[target_class_name] == signal_idx].mean()
fom_std  = train_df[fom_name][train_df[target_class_name] == signal_idx].std()
fom_n_sigmas = 3.0
...
n_output_feats = len(y_train.columns)
...
if 'regress' in model_type:
    ax = plt.subplot(n_rows,n_cols, 4)
    test_dict = {'x1':X_valid[:,0], 'x2':X_valid[:,1], 'm':masses_test, 'y':y_valid, 'pred':y_pred_valid}
...
weighted_df['prob_0'] = model_clf.model.predict(X_weighted)[:,0]
weighted_df['prob_1'] = model_clf.model.predict(X_weighted)[:,1]
if model_type == 'relegator':
    weighted_df['prob_rel'] = model_clf.model.predict(X_weighted)[:,1]

cents, occs, bkgds = hist_softmax_cut_ms(weighted_df, fom_name, fom_min, fom_max, nbins, ax,
                                         sig_limits=(fom_mean - fom_n_sigmas*fom_std,
                                         fom_mean + fom_n_sigmas*fom_std))
raw_signif, pass_signif, n_raw_bkgd, n_raw_sig, n_pass_bkgd, n_pass_sig = compute_signif_binary(weighted_df, fom_name, fom_mean, fom_std, fom_n_sigmas)

y_weighted_1hot = y_weighted_1hot.to_numpy()
cm_weighted = gen_confusion_matrix(y_weighted_1hot.argmax(axis=1),
